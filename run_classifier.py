...
def main(_):
  ...
  if FLAGS.do_eval:
    for id in fsl_task_ids:
      ...
      eval_examples = processor.get_fsl_test_examples(FLAGS.data_dir, id)
      num_actual_eval_examples = len(eval_examples)
      eval_file = os.path.join(FLAGS.output_dir, "fsl_eval.tf_record"+str(id))
      file_based_convert_examples_to_features(eval_examples, label_list, FLAGS.max_seq_length, tokenizer,
                                              eval_file)
      tf.logging.info("***** Running FSL evaluation *****")
      tf.logging.info("  Num examples = %d (%d actual, %d padding)",
                      len(eval_examples), num_actual_eval_examples,
                      len(eval_examples) - num_actual_eval_examples)
      tf.logging.info("  Batch size = %d", FLAGS.eval_batch_size)
